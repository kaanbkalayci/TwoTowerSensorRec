...
@dataclass
class TrainConfig:
    run_name: str = "multi_two_tower_h64_e8"
...
def _load_vehicle_info(
    processed_dir: str | Path,
    prefix: str,
) -> pd.DataFrame | None:
    """Load ground truth vehicle data to map examples to vehicles."""
    try:
        gtruth_vehicle = pd.read_csv(processed_dir / f"{prefix}_ground_truth_vehicle.csv")
        if "vehicle" in gtruth_vehicle.columns:
            return gtruth_vehicle[["datetime", "vehicle"]].drop_duplicates()
    except FileNotFoundError:
        pass
    return None


def _combine_vehicle_utilities(
    examples_index: pd.DataFrame,
    vehicle_info: pd.DataFrame | None,
) -> pd.DataFrame:
    """
    Add vehicle labels to examples for multi-vehicle datasets.
    
    Since both vehicles have identical datetime ranges in this dataset,
    and examples are concatenated (vehicle1 examples + vehicle2 examples),
    we infer vehicle assignment by examining which (time_id, subset) pairs
    have duplicates and assigning first occurrence to vehicle1, second to vehicle2.
    """
    if vehicle_info is None or len(vehicle_info) == 0:
        return examples_index
    
    ex_copy = examples_index.copy()
    vehicles = sorted(vehicle_info["vehicle"].unique())
    
    if len(vehicles) == 1:
        # Single vehicle dataset
        ex_copy["vehicle"] = vehicles[0]
        return ex_copy
    
    # Multi-vehicle: detect which (time_id, subset) pairs appear multiple times
    pair_count = ex_copy.groupby(["time_id", "subset"]).cumcount()
    ex_copy["pair_occurrence"] = pair_count
    
    # Assign vehicles: first occurrence = vehicle1, second = vehicle2, etc.
    ex_copy["vehicle"] = ex_copy["pair_occurrence"].apply(
        lambda x: vehicles[min(x, len(vehicles) - 1)]
    )
    
    return ex_copy.drop(columns=["pair_occurrence"])
...
def load_processed_two_tower_data(
    ...
    *,
    ...
    # Load vehicle information for multi-vehicle datasets
    vehicle_info = _load_vehicle_info(processed_dir, prefix)
    examples_index = _combine_vehicle_utilities(examples_index, vehicle_info)
    ...
    return {
        ...
        "utility_name": utility_name,
        "utility_kwargs": utility_kwargs or {},
        "has_vehicle_info": "vehicle" in examples_index.columns,
    }
...
def build_utility_labels(
    ...
    if utility_name == "saved":
        # Multi-vehicle aware: sum utilities for shared (time_id, subset) pairs
        if "vehicle" in examples_index.columns and "time_id" in examples_index.columns:
            # Use vectorized groupby().transform() to compute sums
            # Create a temporary series with utilities
            temp_df = examples_index[["time_id", "subset"]].copy()
            temp_df["utility"] = saved_y
            
            # Compute sum of utilities for each (time_id, subset) pair
            summed_utilities = temp_df.groupby(["time_id", "subset"])["utility"].transform("sum")
            
            return summed_utilities.to_numpy().astype(np.float32)
        
        # Single vehicle or no time_id: return as-is
        return saved_y.copy()
...
def prepare_standardized_data(data: dict[str, Any], config: TrainConfig) -> dict[str, Any]:
    # In multi-vehicle datasets C_by_time is duplicated (one row-block per vehicle)
    # but example_time_id still indexes only the first block [0, T). Using
    # len(C_by_time) as n_times would push the val/test cut-points beyond the
    # range of any example, producing empty splits.
    n_times = int(data["example_time_id"].max()) + 1
...
def train_one_config(
    ...
    *,
    ...
    # Log multi-vehicle info if available
    if data.get("has_vehicle_info"):
        vehicles = data["examples_index"]["vehicle"].unique()
        print(f"Multi-vehicle dataset detected: {list(vehicles)}")
...
def compute_per_vehicle_utility_stats(
    prepared: dict[str, Any],
) -> dict[str, dict[str, float]] | None:
    """
    Compute per-vehicle utility statistics for multi-vehicle datasets.
    
    Returns dictionary with vehicle names as keys and utility stats as values.
    """
    if not prepared.get("has_vehicle_info"):
        return None
    
    examples_index = prepared["examples_index"]
    y_examples = prepared["y_examples"]
    
    if "vehicle" not in examples_index.columns:
        return None
    
    stats = {}
    for vehicle in examples_index["vehicle"].unique():
        vehicle_mask = examples_index["vehicle"] == vehicle
        vehicle_y = y_examples[vehicle_mask]
        stats[str(vehicle)] = {
            "count": int(vehicle_mask.sum()),
            "mean_utility": float(np.mean(vehicle_y)),
            "std_utility": float(np.std(vehicle_y)),
            "min_utility": float(np.min(vehicle_y)),
            "max_utility": float(np.max(vehicle_y)),
        }
    return stats


def evaluate_per_vehicle_performance(
    model: nn.Module,
    prepared: dict[str, Any],
    split_name: str,
    config: TrainConfig,
    ...
    """
    Evaluate model performance per-vehicle for multi-vehicle datasets.
    
    Returns metrics for each vehicle separately: regression metrics, decision metrics, etc.
    """
    if not prepared.get("has_vehicle_info"):
        return None
    ...
    y = prepared["y_examples"][indices]
    time_id = prepared["example_time_id"][indices]
    ex_index_split = examples_index.iloc[indices].reset_index(drop=True)
    
    per_vehicle_metrics = {}
    for vehicle in ex_index_split["vehicle"].unique():
        vehicle_mask = ex_index_split["vehicle"] == vehicle
        vehicle_scores = scores[vehicle_mask]
        vehicle_y = y[vehicle_mask]
        vehicle_time_id = time_id[vehicle_mask]
        
        value_scores = 1.0 / (1.0 + np.exp(-vehicle_scores)) if config.loss_name == "bce" else vehicle_scores
        metrics = {}
        metrics.update(regression_metrics(vehicle_y, value_scores))
        metrics.update(decision_metrics(vehicle_time_id, vehicle_y, vehicle_scores))
        per_vehicle_metrics[str(vehicle)] = metrics
    
    return per_vehicle_metrics
...
def export_frozen_embeddings(
    ...
    *,
    ...
    # Compute and save per-vehicle utility statistics if available
    per_vehicle_stats = compute_per_vehicle_utility_stats(prepared)
    
    metrics_to_save = dict(result["metrics"])
    if per_vehicle_stats is not None:
        metrics_to_save["per_vehicle_utility_stats"] = per_vehicle_stats
