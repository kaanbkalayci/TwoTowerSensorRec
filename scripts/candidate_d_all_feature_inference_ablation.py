...
RUN_DIR = (
    PROJECT_ROOT
    / "experiments"
    / "feature_candidate_d_two_tower"
    / "fd_drop_rssi_drop_spectral_db_only_h512_d2_e16"
)
TABLE_DIR = PROJECT_ROOT / "reports" / "tables" / "feature_candidate_d_two_tower"
FIG_DIR = PROJECT_ROOT / "reports" / "figures" / "feature_candidate_d_two_tower"
...
def evaluate_ablation(
    ...
    return {
        ...
        "masked_features": ";".join(
            [context_names[i] for i in context_idx] + [action_names[i] for i in action_idx]
        ),
...
def idx(names: list[str], pattern: str) -> list[int]:
    rx = re.compile(pattern)
    return [i for i, name in enumerate(names) if rx.search(name)]
...
band_patterns = [
    ("band_20_80_db", r"band_20_80_db$"),
    ("band_80_160_db", r"band_80_160_db$"),
    ("band_160_400_db", r"band_160_400_db$"),
    ("band_400_900_db", r"band_400_900_db$"),
    ("band_900_2000_db", r"band_900_2000_db$"),
    ("band_2000_3500_db", r"band_2000_3500_db$"),
    ("band_3500_6000_db", r"band_3500_6000_db$"),
]

ablation_specs = [
    ("baseline_no_mask", "none", [], []),
    ("ctx_sensor_geometry", "context", idx(context_names, r"sensor_[xy]_norm$"), []),
    ("ctx_all_band_db", "context", idx(context_names, r"band_\d+_\d+_db$"), []),
    ("act_masks", "action", [], idx(action_names, r"^mask_n")),
    ("act_slot_sensor_geometry", "action", [], idx(action_names, r"^slot\d+_sensor_[xy]_norm$")),
    ("act_subset_size", "action", [], idx(action_names, r"^subset_size$")),
]
ablation_specs.extend(
    (f"ctx_{label}", "context", idx(context_names, pattern), [])
    for label, pattern in band_patterns
)
ablation_specs.extend(
    (f"ctx_feature_{feature_short_name(name)}", "context", [i], [])
    for i, name in enumerate(context_names)
)
ablation_specs.extend(
    (f"act_feature_{name}", "action", [], [i])
...
csv_path = TABLE_DIR / "candidate_d_all_feature_inference_ablation.csv"
ablation_df.to_csv(csv_path, index=False)

plot_df = ablation_df.loc[~ablation_df["ablation"].eq("baseline_no_mask")].head(24).iloc[::-1]
...
ax.set_title("Candidate D inference ablation: groups and individual features")
ax.grid(axis="x", alpha=0.25)
for spine in ["top", "right"]:
    ax.spines[spine].set_visible(False)
fig.tight_layout()
fig.savefig(FIG_DIR / "candidate_d_all_feature_inference_ablation.pdf", bbox_inches="tight")
fig.savefig(FIG_DIR / "candidate_d_all_feature_inference_ablation.png", dpi=300, bbox_inches="tight")
plt.show()

print("\nSaved:")
print(" ", csv_path)
print(" ", FIG_DIR / "candidate_d_all_feature_inference_ablation.pdf")
display(ablation_df)
